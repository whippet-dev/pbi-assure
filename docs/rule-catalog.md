...
| --- | --- | --- | --- | --- | --- |
| `PBI-MODEL-001` | 1.1.0 | Model integrity | Error | Finding | A report-, page-, or visual-scoped PBIR field reference does not resolve to an object in the matching semantic model. Repeated evidence for the same container and object is grouped. |
...
| `PBI-MODEL-005` | 1.0.0 | Model integrity | Warning | Finding | An explicit reference in a supported structured dependency has no target in the same semantic model. Ambiguous references and heuristic DAX resolutions are not reported as this missing-target finding. |
...
| `PBI-COMPAT-002` | 1.0.0 | Compatibility | Warning | Finding | The report explicitly configures a custom theme, but its referenced local resource package item or theme file cannot be resolved or read. Reports with no custom theme, a base theme only, or an unselected registered resource do not trigger this rule. |
| `PBI-ACCESS-001` | 1.1.0 | Accessibility | Warning | Finding | An effectively visible visual included in the tab order has no non-empty literal or dynamic alt-text expression. |
| `PBI-ACCESS-002` | 1.2.0 | Accessibility | Warning | Finding | Two or more effectively visible page items in the same immediate group scope have the same explicit tab-order position, so an equal-ranked item may be skipped during keyboard navigation. |
