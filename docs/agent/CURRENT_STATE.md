...
| **Where a UDF is called from outside the model definition** | **Narrowed and investigated.** An ordinary semantic-model measure calling a UDF is already followed correctly. `PbirReportParser` also retains a report measure's full `Expression`, but `SemanticDependencyAnalyzer` follows only its structured `references.measures`. Existing DAX/UDF extraction is reusable. The supported Desktop case is a live-connected `byConnection` report, which PBI Assure deliberately cannot bind to local model objects; first author the exact fixture in [the investigation](../reviews/report-level-measure-udf-fixture-design.md), then use Desktop bytes for parser evidence and a synthetic local-binding test for graph behaviour. Visual calculations remain unread. [verified in repository; Desktop shape pending] |
